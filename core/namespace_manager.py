...
class NamespaceManager:
    ...
    def __init__(self, api_client):
        ...
        self.ns_cache: Dict[Tuple[str, str],
    ...
    def _load_ns_info(self, family: str, lang: str) -> Dict[int, Dict[str, Set[str] | str]]:
        ...
        try:
            ...
            try:
                query_data = self.api_client.get_namespace_info(
                    family, lang) if self.api_client else {}
            except Exception:
                query_data = {}
            ...
            if not query_data:
                try:
                    url = self.api_client._build_api_url(
                        family, lang) if self.api_client else f"https://{lang}.{family}.org/w/api.php"
                    params = {
                        'action': 'query',
                        'meta': 'siteinfo',
                    ...
                    query_data = (response.json() or {}).get(
                        'query', {}) if response.status_code == 200 else {}
                except Exception:
                    query_data = {}
    ...
    def _common_ns_ids(self) -> List[int]:
        ...
        if not common:
            common = sorted(
                [ns_id for ns_id in DEFAULT_EN_NS.keys() if ns_id > 0])
    ...
    def _primary_label_for_ns(self, family: str, lang: str, ns_id: int) -> str:
        ...
        try:
            ...
            local_prim = self._get_cached_primary_ns_prefix(
                family, lang, ns_id)
            if local_prim:
