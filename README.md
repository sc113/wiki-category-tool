...
- **Чтение**: загрузка списка страниц из текстового поля или файла, получение подкатегорий через API или быстрый переход в PetScan, сохранение результата в `.tsv`.
- **Перезапись**: массовая замена содержимого страниц по TSV, предпросмотр, единый комментарий, отметка «малая правка».
- **Создание**: массовое создание только отсутствующих страниц по TSV (существующие пропускаются), предпросмотр, единый комментарий.
- **Переименование**: переименование страниц с опцией перенаправлений, перенос содержимого категорий (прямые включения и параметры шаблонов), диалоги подтверждения, кэш правил замен шаблонов в `configs/template_rules.json`.
...
- Python 3.10+; зависимости: `PySide6`, `pywikibot`, `requests`, `packaging`
- Запуск: `python -m wiki_cat_tool`
- Для Windows есть портативный `.exe` (см. ниже)
- При запуске автоматически проверяется наличие новой версии на GitHub
...
# или вручную: pip install PySide6 requests pywikibot packaging
```
...
- «Получить подкатегории»: по категории загрузит список подкатегорий через API.
- «PetScan»: откроет готовый запрос в PetScan; если задана глубина, она будет передана в ссылке.
...
- Папка `configs/` располагается рядом с исполняемым файлом (`.exe`) или пакетом (при запуске из исходников). Внутри:
  ...
  - `update_settings.json` — настройки проверки обновлений (пропущенные версии).
  - `apicache/` — кэш префиксов пространств имён и другие вспомогательные JSONы.

## Проверка обновлений
- При каждом запуске приложение автоматически проверяет наличие новой версии на GitHub (через 1 секунду после старта).
- Если доступна новая версия, появляется диалоговое окно с информацией и ссылкой для скачивания.
- Можно пропустить конкретную версию (установив галочку в диалоге) — уведомление для неё больше не появится.
- Проверка происходит в фоновом режиме и не блокирует работу приложения.
- При отсутствии интернета или ошибках API проверка просто игнорируется.
